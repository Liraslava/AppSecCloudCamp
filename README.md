...
В данной диаграмме есть несколько потенциальных уязвимостей:
1. Данные пользователей хранятся в БД, как и настройка уведомлений, а связь БД с Бэкендом осуществляется напрямую, без дополнительной сущности, которая бы контролировала взаимодействие между этими модулями.
2. Отсутсвует модуль контроля доступа к модулю Auth. (Я бы связала модуль Auth с модулем User - не понимаю, как на этой dfd происходит авторизация пользователя).

Методы решения:

1. Реализовать строгий контроль доступа, включающий авторизацию на разных уровнях и проверку подлинности.
2. Внедрить шифрование данных при хранении и передаче, использовать безопасные протоколы связи (если не внедрено)

[img/dfd.drawio.png]

Вопросы, которые стоит задать разработчикам:
1. Какие методы шифрования используются при хранении и передачи данных (если используются)?
2. Как реализован контроль доступа к функционалу ит данным на уровне Backend?
3. Какие механизмы авторизации и аутентификации используются в модуле Auth?
4. Какие меры безопасности были приняты для защиты данных пользователей, хранящихся в PostgreAQL и S3?
5. Как производится мониторинг и реагиррование на инцинденты безопасности?
